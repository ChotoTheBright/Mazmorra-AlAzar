...
const START_KEY = 21
const END_KEY = 108
const WhiteKeyScene = preload("res://Scenes/white_key.tscn")
const BlackKeyScene = preload("res://Scenes/black_key.tscn")

@onready var white_keys = $KeysWhite
@onready var black_keys = $KeysBlack

var key: MusicKey #PianoKey
var music_key_dict := Dictionary()
...
func _input(input_event):
	...
	key = music_key_dict[midi_event.pitch]
...
func _create_music_key(pitch_index):
	var note_index = _pitch_index_to_note_index(pitch_index)
	var music_key: MusicKey
	if _is_note_index_sharp(note_index):
		music_key = BlackKeyScene.instantiate()
		black_keys.add_child(music_key)
	else:
		music_key = WhiteKeyScene.instantiate()
		white_keys.add_child(music_key)
		if _is_note_index_lacking_sharp(note_index):
			_add_placeholder_key(black_keys)
	music_key.setup(pitch_index)
	return music_key
