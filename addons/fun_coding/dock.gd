@tool
extends Control
#Ridiculous Coding Dock
const BASE_XP: int = 50 #
const STATS_FILE: String = "user://ridiculous_xp.ini"

var explosions: bool = true
var blips: bool = false#true
var chars: bool = true
var shake: bool = false#true
var sound: bool = false#true
...
@onready var pet_checkbox: CheckButton = $VBoxContainer/GridContainer/petCheckbox
...
func connect_checkboxes():
	...
	pet_checkbox.toggled.connect(func(toggled):
		pet = toggled
		save_checkbox_state()
	)
...
func save_checkbox_state():
	...
	stats.set_value("settings", "pet", pet)
	stats.save(STATS_FILE)
...
func load_checkbox_state():
	if stats.load(STATS_FILE) == OK:
		...
		pet_checkbox.button_pressed = stats.get_value("settings", "pet", true)
	else:
		...
		pet_checkbox.button_pressed = pet
